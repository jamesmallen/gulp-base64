...
var rImages = /([\s\S]*?)(url\(([^)]+)\)|xlink:href="([^"]+)")(?!\s*[;,]?\s*\/\*\s*base64:skip\s*\*\/)|([\s\S]+)/img;
...
module.exports = (function () {
  ...
  /**
   * Takes a CSS or SVG file as input, goes through it line by line, and base64
  ...
  exports.stylesheet = function(file, opts, done) {
    ...
    if (opts.debug) {
      console.log("Examining", file.path);
    }

    var deleteAfterEncoding = opts.deleteAfterEncoding;
    var src = file.contents.toString();
    var result = "";
    var match, img, line, tasks, group, rawUrl, urlWrapper;
    ...
    function(complete) {
      ...
      //    group[3] will hold the contents of the url declaration if it is a css url
      //    group[4] will hold the contents of the url declaration if it is a xlink:href url
      //    group[5] will be undefined
      // if there is no other url to be processed, then group[1-3] will be undefined
      //    group[5] will hold the entire string
      ...
      if(group[5] == null) {
        ...
        if (typeof group[3] !== 'undefined') {
          rawUrl = group[3].trim();
          urlWrapper = 'url({{url}})';
        } else {
          rawUrl = group[4].trim();
          urlWrapper = 'xlink:href="{{url}}"'
        }
        img = rawUrl
          .replace(rQuotes, "")
        ...
        if(cache[img]) {
          ...
          exports.image(loc, opts, function (err, resp, cacheable) {
            if (err == null) {
              var url = urlWrapper.replace('{{url}}', resp);
              result += url;
